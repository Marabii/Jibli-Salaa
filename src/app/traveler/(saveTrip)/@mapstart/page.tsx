...
type PositionKey = "from" | "to";

export default function TravelerMap() {
  const pos: PositionKey = "from";
  ...
  const travelerTripState: TravelerTripState["value"] = useAppSelector(
    (state) => state.travelerTrip.value
  );

  // Ensure a marker is set on page load if the store has data
  useEffect(() => {
    const tripLocation: AddressObject = travelerTripState[pos];
    if (
